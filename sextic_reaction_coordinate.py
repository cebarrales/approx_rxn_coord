...
# coef_matrix = Matrix([-640.011562944536, 2108.94580707296,
#                       -1788.09460758944, -202.381954261866, 527.432317722878]) endo
# coef_matrix = Matrix([-698.0288158535812, 2000.4097266407093,
#                       -1657.308511379374, 30.403106250946294, 230.57449434129944]) xts = 0.38
# coef_matrix = Matrix([-845.7973482679712, 2493.1951691064887,
#                       -1767.4200279106558, -561.5560584262729, 681.5782654984109]) xts = 0.4972
# coef_matrix = Matrix([-588.785201131717, 1873.4092567113757,
#                       -1701.9579018291126, 152.84883805097772, 245.4750081984752]) xts=0.4930
...
def initial_guess(erxn):
    ...
    if erxn > 0:
        ...
        #                     230.57449434129944])
        coef_matrix = Matrix(
            [-675.0237837035049, 2201.994551010687, -2156.699474750905, 421.5304312838907, 201.18827615983392])
    return coef_matrix
...
def final_results(opt_coeff, xts, convergence, save_data, output_file, j):
    ...
    if save_data == "yes":
        ...
        output_file.write('\n=====================================================\n\n')
        output_file.write('Arc lenght           = ' + str(round(arc_len, 2)) + '\n')
        output_file.write('\n===============Reaction Force Analysis===============\n\n')
        output_file.write('F_min                = ' + str(round(cp1, 4)) + '\n')
        output_file.write('F_max                = ' + str(round(cp2, 4)) + '\n\n')
        output_file.write(
            'W1                   = ' + str(round(w1, 2)) + ' (' + str(round(w1 * 100 / (w1 + w2), 2)) + '% )' + '\n')
        output_file.write(
            'W2                   = ' + str(round(w2, 2)) + ' (' + str(round(w2 * 100 / (w1 + w2), 2)) + '% )' + '\n')
        output_file.write(
            'W3                   = ' + str(round(w3, 2)) + ' (' + str(round(w3 * 100 / (w3 + w4), 2)) + '% )' + '\n')
        output_file.write(
            'W4                   = ' + str(round(w4, 2)) + ' (' + str(round(w4 * 100 / (w3 + w4), 2)) + '% )' + '\n')
        output_file.write('\n=====================================================\n')
    else:
        print('\n====================Final Results====================\n')
        ...
        print('\n=====================================================\n')
        print('Arc lenght           =', round(arc_len, 2))
        print('\n===============Reaction Force Analysis===============\n')
...
def arc_lenght(coef):
    ...
    def f(x):
        fun = np.sqrt((6 * coef[0] * x ** 5 + 5 * coef[1] * x ** 4 + 4 * coef[2] * x ** 3
                       + 3 * coef[3] * x ** 2 + 2 * coef[4] * x) ** 2 + 1)
        return fun
    a_i = (float(1 - 0)/1000)
    sum_of_areas = 0
    for i in range(1000):
