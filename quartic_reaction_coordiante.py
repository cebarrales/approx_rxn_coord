...
def arc_lenght(coef):
    ...
    def f(x):
        fun = np.sqrt((4 * coef[0] * x ** 3 + 3 * coef[1] * x ** 2
                       + 2 * coef[2] * x) ** 2 + 1)
        return fun
    a_i = (float(1 - 0)/10000)
    sum_of_areas = 0
...
def quartic_plot1(a_i, b_i, c_i, want_plot=WANT_PLT, name=DEFAULT_NAME):
    """ Collect the data of a simple QRC analysis and plot it """
